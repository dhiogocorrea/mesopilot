...
export function Logo({ className }: { className?: string }) {
  return (
    <span className={`inline-flex items-center gap-3 ${className ?? ""}`}>
      <LogoMark className="h-9 w-9 shrink-0 text-ink" title="Meso505" />
      {/* The digits take the accent and stay on the sans: `display-face` is a
          condensed stack, and condensed figures are the one thing this app
          never sets — see the note in globals.css. */}
      <span className="display-face text-[2rem] font-bold leading-none tracking-tight">
        Meso
        <span className="font-sans text-accent tabular-nums">505</span>
      </span>
    </span>
  );
}
